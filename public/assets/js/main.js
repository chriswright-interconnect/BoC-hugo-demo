(() => {
  ...
  var setWasProcessed = (element) => setData(element, processedDataName, processedDataValue);
  var getWasProcessed = (element) => getData(element, processedDataName) === processedDataValue;
  ...
  var replaceExtToWebp = (value, condition) => condition ? value.replace(/\.(jpe?g|png)/gi, ".webp") : value;
  var detectWebp = () => {
  ...
  var setSourcesInChildren = function(parentTag, attrName, dataAttrName, toWebpFlag) {
  ...
  var setAttributeIfValue = function(element, attrName, value, toWebpFlag) {
    if (!value) {
      return;
    }
  ...
  var setSourcesBgImage = (element, settings) => {
    const toWebpFlag = supportsWebp && settings.to_webp;
    const srcDataValue = getData(element, settings.data_src);
    if (srcDataValue) {
  ...
  var callCallback = function(callback, argument) {
    if (callback) {
      callback(argument);
    }
  };
  ...
  var removeListeners = function(element, loadHandler, errorHandler) {
    element.removeEventListener(loadString, loadHandler);
    element.removeEventListener(errorString, errorHandler);
  };
  var addOneShotListeners = function(element, settings) {
    const onLoad = (event) => {
  ...
  var onEvent = function(event, success, settings) {
    const element = event.target;
    removeClass(element, settings.class_loading);
    addClass(element, success ? settings.class_loaded : settings.class_error);
    callCallback(
  ...
  var isIntersecting = (element) => element.isIntersecting || element.intersectionRatio > 0;
  ...
  LazyLoad.prototype = {
    ...
    update: function(elements) {
      ...
      this._elements = purgeElements(Array.prototype.slice.call(nodeSet));
  ...
  var lazyload_es2015_default = LazyLoad;

  // ns-hugo:C:\Hugo\BoC-hugo-demo\assets\js\components\carousel.js
  var import_jquery = __toESM(require_jquery());
  var import_slick_carousel = __toESM(require_slick());
  var $carousel = (0, import_jquery.default)(".quotes-slider__list");
  if ($carousel.length) {
    $carousel.slick({
      useTransform: true,
      dots: false,
